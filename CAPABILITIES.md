...
Search execution is explicit: field edits remain drafts until the operator selects Search or presses Enter. Execution opens a dedicated Search results tab inside the People workspace. Results are compact contact cards labeled Customer or Internal with a blank photo, person name, and company. Selecting a card opens the existing full person profile tab. Directory search, Search results, and multiple person profiles can remain open simultaneously without browser tabs or overlays; result and person tabs can be closed independently.
